...
ESLint では、ソースコードからASTを生成するといったパース処理を、`espree`というparserを使用して行っています。
...
まず、テストケースとして、どのようなコードを正常として、どのようなコードを異常とするのかを考えます。その後、正常系、異常系それぞれのコードのASTの内容を見ながら、ルールを実装していき、最後にテストの実装を行います。  
今回実装するルールは非常に単純なものですが、ルールの複雑性が増しても、基本的な流れは同じです。  

---

ではまず、テストケースについてです。
今回の、if文に必ずBlockを使用するというルールのテストケースを考えると、こちらのように、Block が使われている。つまり`{}`で囲われている場合は正常なケースとし、`{}`で囲われていない場合はエラーとします。  

こちらのコードを元に、実際のASTを見てみます。  
今回は非常に簡潔にした AST をスライドに載せていますが、実際に開発する上で、コードのASTを見る際には、 [ast explorer](https://ast-explorer.dev/) というサイトが非常に便利です  
...
ここまでで、ルールの実装が完了しましたので、最後にテストを実装します。  
...
ESLint では espree という parser を使用して、ソースコードからASTを生成しております。  
しかしこのespreeというparserは、JavaScriptのparserであるため、TypeScriptコードをparseすることはできません。  
そこで、`typescript-eslint/parser`を使用して、TypeScriptコードをparseし、TypeScriptコードをサポートしたASTを生成します。

補足として、ここで生成される AST は、`typescript`で定義されている AST ではなく、`typescript-eslint` で定義されている AST です。  
ここには`typescript`で定義されているASTの情報の他に、`typescript-eslint`独自の情報が含まれています。
...
架空のシナリオとして「カスタムの`Error`クラスが、`Error`クラスを継承しているかを判断し、継承していない場合は規約違反とする」というものを考えます。
...
<!-- ここから -->
今回は、`SampleError`や`BaseError`など、クラス名の末尾に 「Error」 とつく場合、`Error`クラスを継承している場合は正常としています。
直接`Error`クラスを継承している場合と、間接的に`Error`クラスを継承している場合、どちらも正常ケースとしています。  
一方で、Errorクラスを継承していない場合は、異常としています。

このルールの実装の内容としては、このようになります。
...
では、まず`1`の定義されたクラス名の末尾が`Error`かを判定するところから実装していきますが、そのためにはまず、クラスを定義するコードがどのような AST になっているかを把握する必要があります。  
そのため、先ほどの`ast-explorer`を使用して、`Sample`という名前のクラスを定義した時のASTを見てみます。

ここではかなり簡潔化したASTをスライドに載せていますが、ClassDeclarationsの`id`プロパティの`name`プロパティに、クラス名が格納されていることがわかります。  
